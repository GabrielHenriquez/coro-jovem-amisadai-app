...
} from "./presenceComponentsHtmlContent";

export interface PresenceComponentsData {
  year: string;
  eventsLength: number;
  totalPresent: number;
  soprano: PresenceComponentWithCount[];
  contralto: PresenceComponentWithCount[];
  tenor: PresenceComponentWithCount[];
  baixo: PresenceComponentWithCount[];
}

const annualPresenceComponentsHtmlContent = (data: PresenceComponentsData) => {
  return `<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <title>Relatório Anual de Presenças - Coro Jovem Amisadai</title>
    <style>
        body {
...
<body>
    <div class="container">
        <div class="header">
            ...
            <h2>Relatório Anual de Presenças - ${data.year}</h2>
            <p class="header-info">
        ...
        <div class="summary">
            <h3>Resumo Geral do Ano</h3>
            <p><strong>Chamadas no ano:</strong> ${data.eventsLength}</p>
            <p><strong>Total de Componentes com Presenças:</strong> ${
              data.totalPresent
            }</p>
            ...
            <p><strong>Total de Presenças Registradas no Ano:</strong> ${
        ...
        ${
          data.totalPresent > 0
            ? `
        <div class="section-title">Componentes com Presenças no Ano (Ordenados por Maior Frequência)</div>
                        ...
                        .map(
                          (item) =>
                            `<li><span class="component-name">${
                             ...
                             <span class="presence-count">${item.count} ${
                              item.count === 1 ? "presença" : "presenças"
                            }</span></li>`
                        )
                        .join("")}</ul>`
                    : '<div class="empty-naipe">Nenhum componente presente</div>'
                }
            </div>
        ...
        `
            : `
        <div class="section-title">Nenhuma Presença Registrada</div>
        <div class="summary">
            <p>Não há presenças registradas para este ano.</p>
        </div>
        `
        }
...
export default annualPresenceComponentsHtmlContent;
