...
import { IEventCard } from "@features/calls/domain/entities/Events";
import { FirebaseEventsService } from "../domain/services/FirebaseEventsService";
...
export const useEventsQueries = () => {
  ...
  const deleteEventMutation = useMutation({
    mutationFn: async (event: IEvent) => {
      if (!event.id || !event.type || !event.date || !event.numberSearchDoc) {
        throw new Error('Dados do evento incompletos para exclusão');
      }
      
      const key = `${event.type}-${event.date}-${event.numberSearchDoc}`;
      await Promise.all([
        repository.deleteEvent(event.id),
        repository.deleteDotInDB({ key, date: event.date }),
      ]);
    },
  });
